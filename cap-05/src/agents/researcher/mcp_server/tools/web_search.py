...
async def search_competitors(market: str, location: str = "Brasil") -> str:
    ...
    for r in all_results:
        if r["url"] not in seen_urls:
            seen_urls.add(r["url"])
            unique_results.append({"title": r["title"], "url": r["url"], "snippet": r["content"][:300]})

    return json.dumps({"market": market, "location": location, "results": unique_results[:10]}, ensure_ascii=False)
